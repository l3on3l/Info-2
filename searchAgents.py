...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        #retornamos una tupla con el estado inicial junto a una lista para marcar las esquinas visitadas
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        # state[1]-> lista de los estados de las esquinas visitadas
        #verificamos si ya visitamos las cuatros esquinas correspondientes
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            #fragmento de codigo sugerido para saber si la nueva posicion "golpea" una pared
            x, y = currentPosition
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            hitsWall = self.walls[nextx][nexty]
            #verificamos si no golpeamos una pared
            if not hitsWall:
                nextState = (nextx, nexty)
                # si el sgte. estado es una esquina no visitada entonces "actualizamos" la lista
                if nextState in self.corners and nextState not in visitedCorners:
                        visitedCorners = visitedCorners + [nextState]
                # agregamos el nodo sucesor (nuevos estados, accion, costo), mantenemos el costo igual a 1 para todos.
...
def cornersHeuristic(state, problem):
    ...
    # desestructuramos nuestro estado
    currentPosition, visitedCorners = state
    # obtenemos todas las esquinas que aun no visitamos para saber cual de las que aun no visitamos
    ...
    while unvisitedCorners:
        ...
        currentCorner = unvisitedCorners[0]
        heuristicDistance = util.manhattanDistance(currentPosition, currentCorner)
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        x,y = state
        "*** YOUR CODE HERE ***"
        #con asList() pasamos self.food como lista con las coordenadas de las comidas.
